...
const templateFiles = fs
  .readdirSync(path.resolve(environment.paths.source, "templates", "pages"))
  .filter((file) => [".html", ".ejs"].includes(path.extname(file).toLowerCase()))
  .map((filename) => ({
    input: path.resolve(environment.paths.source, "templates", "pages", filename),
...
module.exports = {
  ...
  plugins: [
    ...
    //     new HTMLWebpackPlugin({
    //       inject: true,
    //       template: path.resolve(environment.paths.source, `${page}.html`), // `./`,
    //       filename: `${page}.html`,
    //       chunks: [page],
    //     })
    // )
